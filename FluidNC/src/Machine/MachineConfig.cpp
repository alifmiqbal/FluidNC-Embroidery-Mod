...
namespace Machine {
    void MachineConfig::group(Configuration::HandlerBase& handler) {
        ...
        if (_embroidery) {
            handler.enterFactory("embroidery", *_embroidery);
        }
    ...
    void MachineConfig::afterParse() {
        ...
        if (_embroidery == nullptr) {
            _embroidery = EmbroideryController::getInstance();
        }
        // Initialize Embroidery Controller (pins, interrupts, queues)
        _embroidery->begin();
