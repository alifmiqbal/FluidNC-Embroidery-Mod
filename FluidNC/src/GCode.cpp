...
#include "Embroidery/EmbroideryController.h"
    ...
    if (gc_state.modal.motion != Motion::None) {
        if (axis_command == AxisCommand::MotionMode) {
            ...
            if (gc_state.modal.motion == Motion::Linear) {
                // EMBROIDERY INTERCEPTION (G1)
                if (embroidery && embroidery->isEnabled()) {
                    // Check if X or Y is present
                    if (axis_words & (bitnum_to_mask(X_AXIS) | bitnum_to_mask(Y_AXIS))) {
                         float dx_mm = gc_block.values.xyz[X_AXIS] - gc_state.position[X_AXIS];
                         float dy_mm = gc_block.values.xyz[Y_AXIS] - gc_state.position[Y_AXIS];
                         
                         float steps_per_mm_x = Machine::Axes::_axis[X_AXIS]->_stepsPerMm;
                         float steps_per_mm_y = Machine::Axes::_axis[Y_AXIS]->_stepsPerMm;
                         
                         int32_t steps_x = (int32_t)(dx_mm * steps_per_mm_x);
                         int32_t steps_y = (int32_t)(dy_mm * steps_per_mm_y);
                         
                         // Queue stitch (Blocking if full)
                         embroidery->queueStitch(steps_x, steps_y, 600); 
                    } else {
                        // Z-only move (or other axis). Ignore physical movement in embroidery mode.
                        // ets_printf("Ignored non-XY move\n");
                    }
                    // Always update position so FluidNC thinks it moved
                    copyAxes(gc_state.position, gc_block.values.xyz);
                } else {
                    mc_linear(gc_block.values.xyz, pl_data, gc_state.position);
                }
            } else if (gc_state.modal.motion == Motion::Seek) {
                // EMBROIDERY INTERCEPTION (G0)
                if (embroidery && embroidery->isEnabled()) {
                    if (axis_words & (bitnum_to_mask(X_AXIS) | bitnum_to_mask(Y_AXIS))) {
                         float dx_mm = gc_block.values.xyz[X_AXIS] - gc_state.position[X_AXIS];
                         float dy_mm = gc_block.values.xyz[Y_AXIS] - gc_state.position[Y_AXIS];
